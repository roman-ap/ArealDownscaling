...
ggplot(k_data,aes(x=as.vector(ks),y=marginalk))+
  ...
  expand_limits(x=c(1e-07,3e-07))+
  labs(x=TeX("$\\kappa$"),y=TeX("$p(\\kappa | \\mathbf{y})$"))+
  theme_bw()
# Weights
k_weights4<-(ks[2]-ks[1])*marginalk
save(k_weights4,file="k_weights4.RData")
...
x_means4<-foreach(k=as.vector(ks),.combine='cbind',.packages=c('Matrix')) %dopar% {
  t=1e-02
  Q_x=bdiag(Q_beta,k*W)
  Q=forceSymmetric(Q_x+t*BtB)
  L=Cholesky(Q)
...
stopCluster(cl)
# RData
save(x_means4,file="x_means4.RData")
...
registerDoRNG(27)
registerDoParallel(cl)
x_vars4<-foreach(k=as.vector(ks),.combine='cbind',.packages=c('Matrix')) %dopar% {
  t=1e-02
  Q_x=bdiag(Q_beta,k*W)
  Q=forceSymmetric(Q_x+t*BtB)
  L=Cholesky(Q,LDL=F)
...
stopCluster(cl)
# RData
save(x_vars4,file="x_vars4.RData")
